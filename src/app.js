...
function appObj()
{
     ...
     * 当生命值归零时调用
     */
    function showRevivePanel()
    {
        console.log('[Game] 生命归零，显示复活面板');
        ...
        // 显示复活面板
        if (typeof RevivePanel !== 'undefined') {
            RevivePanel.show(
                // 复活成功回调
                function() {
                    reviveByAd();
    ...
    function reviveByAd()
    {
        console.log('[Game] 广告复活成功，恢复游戏');
        
        // 恢复一颗生命
        hitNum = 1;
        heartMc.removeAllChildren();
        addHeart(heartMc, hitNum);
        heartMc.y = 100;
        
        // 重置小球状态
        ball.gotoAndStop(0);
        ball.x = ballXPosArr[ballXId]; // 保持在当前位置
        ball.y = 820;
        ball.scaleX = ball.scaleY = 1;
        ...
        // 显示复活成功提示
        showReviveSuccessTip();
    }
    
    /**
     * 显示复活成功提示
     */
    function showReviveSuccessTip()
    {
        // 创建复活成功提示
        var tip = document.createElement('div');
        tip.id = 'reviveSuccessTip';
    ...
    function saveGameRecord(gameScore)
    {
        var records=localStorage.getItem('game_records');
        var recordList=records?JSON.parse(records):[];
        ...
        localStorage.setItem('game_records',JSON.stringify(recordList));
